...
from trainer_actvae_p2inetwork import Trainer
...
def train(config, opts):
    ...
    _, _ = trainer.resume(opts.vae_path, hyperparameters=config)
    if opts.resume:
        iterations, start_epoch = trainer.resume2(checkpoint_directory, hyperparameters=config)
    else:
        iterations, start_epoch = 0, 0
    
    batch_time = AverageMeter()
    data_time = AverageMeter()
    loss_recon_target = AverageMeter()
    loss_vgg_target = AverageMeter()
    loss_feature = AverageMeter()

    loss_gen_adv_pair = AverageMeter()
    loss_dis_adv_pair = AverageMeter()

    loss_gen_adv_pair2 = AverageMeter()
    loss_dis_adv_pair2 = AverageMeter()
    ...
    for epoch in range(start_epoch, train_epoch, 1):
        for batch_idx, (source_image, target_image1, target_image2, target_image3, target_image4, target_image5,
            ...
            with Timer("Elapsed time in update: %f"):
                trainer.dis_update(source_image, target_image1, target_image2, target_image3, target_image4, target_image5, 
            ...
            loss_recon_target.update(trainer.loss_recon_target.cpu().item(), source_image.size(0))
            loss_vgg_target.update(trainer.loss_vgg_target.cpu().item(), source_image.size(0))
            loss_feature.update(trainer.loss_feature.cpu().item(), source_image.size(0))

            loss_gen_adv_pair.update(trainer.loss_gen_adv_pair.cpu().item(), source_image.size(0))
            loss_dis_adv_pair.update(trainer.loss_dis_adv_pair.cpu().item(), source_image.size(0))

            loss_gen_adv_pair2.update(trainer.loss_gen_adv_pair2.cpu().item(), source_image.size(0))
            loss_dis_adv_pair2.update(trainer.loss_dis_adv_pair2.cpu().item(), source_image.size(0))

            if (iterations + 1) % config['log_iter'] == 0:
                print_str = 'Epoch: [{0}][{1}/{2}]\t' \
                            'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t' \
                            'Data Time {data_time.val:.3f} ({data_time.avg:.3f})\t' \
                            'Loss_recon_target {loss_recon_target.val:.4f} ({loss_recon_target.avg:.4f})\t' \
                            'Loss_vgg_target {loss_vgg_target.val:.4f} ({loss_vgg_target.avg:.4f})\t' \
                            'Loss_feature {loss_feature.val:.4f} ({loss_feature.avg:.4f})\t' \
                            'Loss_gen_adv_pair {loss_gen_adv_pair.val:.4f} ({loss_gen_adv_pair.avg:.4f})\t' \
                            'Loss_dis_adv_pair {loss_dis_adv_pair.val:.4f} ({loss_dis_adv_pair.avg:.4f})\t' \
                            'Loss_gen_adv_pair2 {loss_gen_adv_pair2.val:.4f} ({loss_gen_adv_pair2.avg:.4f})\t' \
                            'Loss_dis_adv_pair2 {loss_dis_adv_pair2.val:.4f} ({loss_dis_adv_pair2.avg:.4f})\t' \
                    .format(epoch, batch_idx, len(train_loader), batch_time=batch_time,
                            data_time=data_time,
                            loss_recon_target=loss_recon_target,
                            loss_vgg_target=loss_vgg_target,
                            loss_feature=loss_feature,
                            loss_gen_adv_pair=loss_gen_adv_pair, loss_dis_adv_pair=loss_dis_adv_pair,
                            loss_gen_adv_pair2=loss_gen_adv_pair2, loss_dis_adv_pair2=loss_dis_adv_pair2)
                print(print_str)
...
def test2(config, opts):
    ...
    output_directory = os.path.join(opts.output_path + "/outputs", model_name)
    checkpoint_directory, image_directory = prepare_sub_folder(output_directory)
    results_directory = image_directory.replace('images', 'results')
    ...
    output_im_real = 'outputs/Penn_actvae_p2inetwork/results_image_real'
    output_im_fake = 'outputs/Penn_actvae_p2inetwork/results_image_fake'
    output_gif_real = 'outputs/Penn_actvae_p2inetwork/results_gif_real'
    output_gif_fake = 'outputs/Penn_actvae_p2inetwork/results_gif_fake'
    ...
    for batch_idx, (source_image, target_image1, target_image2, target_image3, target_image4, target_image5,
        ...
        with Timer("Elapsed time in update: %f"):
            pose_seq, image_seq, \
            pose_seq_real, image_seq_real = trainer.test_forward(source_image, target_image1, target_image2,
        ...
        for mm in range(batch_size):
            img_frame = untransformTensor(image_seq[mm].data.cpu())
            img_frame = np.moveaxis(img_frame, 0, 2).astype(np.uint8).copy()

            img_frame_real = untransformTensor(image_seq_real[mm].data.cpu())
            img_frame_real = np.moveaxis(img_frame_real, 0, 2).astype(np.uint8).copy()

            if not(mm==0):
                img_collection_real += [img_frame_real]
                img_collection_fake += [img_frame]
        
        imageio.mimsave(os.path.join(output_gif_real, '%04d' % seq_id + '_' + '%04d.gif' % (image_id)), img_collection_real, fps=2)
        imageio.mimsave(os.path.join(output_gif_fake, '%04d' % seq_id + '_' + '%04d.gif' % (image_id)), img_collection_fake, fps=2)
        count += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vae_path', type=str, default='Penn_pose_pred_time_2_test/checkpoints', help="outputs path")
