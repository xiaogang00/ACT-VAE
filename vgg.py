...
def vgg19(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['vgg19'], 'models'))
    return model


class VGG_Activations2(nn.Module):
    def __init__(self, feature_idx):
        super(VGG_Activations2, self).__init__()
        vgg_network = vgg19(pretrained=True)
        features = list(vgg_network.features)
        self.features = nn.ModuleList(features).eval()
        self.idx_list = feature_idx
