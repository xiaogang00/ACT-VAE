...
def random_affine(img, mask, targets, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
    ...
    if mask is not None:
        maskw = cv2.warpPerspective(mask, M, dsize=(height, height), flags=cv2.INTER_NEAREST, borderValue=0)
    else:
        maskw = None
...
def random_affine2(img, img_list, mask, targets, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
                  borderValue=(123.7, 116.3, 103.5), all_bbox=None):
    border = 0
    height = max(img.shape[0], img.shape[1]) + border * 2
    ...
    for mm in range(len(img_list)):
        im_this = cv2.warpPerspective(img_list[mm], M, dsize=(height, height), flags=cv2.INTER_LINEAR,
                                      borderValue=borderValue)  # BGR order borderValue
        other_list.append(im_this)
    if mask is not None:
