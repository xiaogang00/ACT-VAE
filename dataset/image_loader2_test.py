...
class ReferDataset_test(data.Dataset):
    ...
    def pull_item(self, img_file):
        img_file = img_file.replace('/mnt/backup/project/xgxu/landmark_data/Penn_Action_new/frames', 'data/Penn_frames')
        img_path = img_file
    ...
    def __getitem__(self, idx):
        ...
        img_path_all = source_img_name
        id_this1 = img_path_all.split('/')[-2]
        id_this2 = img_path_all.split('/')[-1].split('.')[0]
        id_this1 = int(id_this1)
        id_this2 = int(id_this2)

        img_path_all = target_img_name1
        id_this3 = img_path_all.split('/')[-1].split('.')[0]
        id_this3 = int(id_this3)
        gap = id_this3 - id_this2

        seq_id = torch.Tensor([id_this1])
        image_id = torch.Tensor([id_this2])
        ...
        return source_img, target_img1, target_img2, target_img3, target_img4, target_img5, \
               ...
               point_target1, point_target2, point_target3, point_target4, point_target5, seq_id, image_id, gap
